...
function metricsBlock(result: PostMortemResult): string {
  ...
  if (pi) {
    lines.push(`- **Deal Risk Score:** ${pi.deal_risk_index} (${pi.risk_tier})`);
    lines.push(`- **Department Friction (DFI):** ${pi.multi_department_friction}`);
    lines.push(`- **Dispersion:** ${pi.stakeholder_dispersion_index}`);
    lines.push(`- **Stall signals:** ${pi.dialogue_stall_score}`);
    if (pi.authority_gap_flag) lines.push("- **Authority gap:** Yes");
  } else if (triage && triage.department_friction_index > 0) {
    lines.push(`- **Department friction index:** ${triage.department_friction_index}`);
  }
  if (vs) {
    lines.push(`- **Viability:** ${vs.state} (${vs.viability_score}/100)`);
  }
